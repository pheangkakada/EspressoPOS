document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateInvoiceDateDisplay() {
    ...
    const invoiceDateElement = document.querySelector(
      ".table-selector-group h3",
    );
  ...
  function updateInvoiceNumberDisplay(invoiceId = null, nextInvoiceId = null) {
    const invoiceNumberElement = document.getElementById(
    ...
    async getMenuItems() {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Menu API failed: ${response.status} ${response.statusText}`,
          );
        }
    ...
    return [
      {
        ...
        isActive: true,
        image:
          "https://images.unsplash.com/photo-1554866585-cd94860890b7?w=200&h=130&fit=crop",
      },
      {
        _id: "2",
        id: "2",
        ...
        isActive: true,
        image:
          "https://images.unsplash.com/photo-1622483767028-3f66f32aef97?w=200&h=130&fit=crop",
      },
      {
        _id: "3",
        id: "3",
        ...
        isActive: true,
        image:
          "https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=200&h=130&fit=crop",
      },
      {
        _id: "4",
        id: "4",
        ...
        isActive: true,
        image:
          "https://images.unsplash.com/photo-1576107232684-1279f390859f?w=200&h=130&fit=crop",
      },
      {
        _id: "5",
        id: "5",
        ...
        isActive: true,
        image:
          "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=200&h=130&fit=crop",
      },
      {
        _id: "6",
        id: "6",
        ...
        isActive: true,
        image:
          "https://images.unsplash.com/photo-1561047029-3000c68339ca?w=200&h=130&fit=crop",
      },
    ];
  }
  ...
  const subTotalDisplay = document.querySelector(
    ".calc-row:nth-child(1) span:nth-child(2)",
  );
  const discountDisplay = document.querySelector(
    ".discount-row span:nth-child(2)",
  );
  ...
  const currentTableDisplay = document.querySelector(
    ".table-selector-group h2",
  );
  ...
  const changeDisplayUSDContainer = document.getElementById(
    "changeDisplayUSDContainer",
  );
  const changeDisplayKHRContainer = document.getElementById(
    "changeDisplayKHRContainer",
  );

  // Delivery Modal Elements
  const deliveryModal = document.getElementById("deliveryModal");
  const deliveryAppBtns = deliveryModal
    ? deliveryModal.querySelectorAll(".app-btn")
    : [];
  ...
  const discountTypeBtns = discountTypeButtons
    ? discountTypeButtons.querySelectorAll(".discount-type-btn")
    : [];
  const itemModalTotalPriceDisplay = document.getElementById(
    "itemModalTotalPrice",
  );
  ...
  const sugarSelectionBtns = document
    .getElementById("sugarSelection")
    .querySelectorAll(".selection-btn");
  const modalQtyDecreaseBtn = itemDetailModal
    ? itemDetailModal.querySelector(
        '.item-controls-modal .qty-btn[data-action="decrease"]',
      )
    : null;
  const modalQtyIncreaseBtn = itemDetailModal
    ? itemDetailModal.querySelector(
        '.item-controls-modal .qty-btn[data-action="increase"]',
      )
    : null;
  ...
  const globalDiscountValueInput = document.getElementById(
    "globalDiscountValue",
  );
  const globalDiscountTypeButtons = document.getElementById(
    "globalDiscountTypeButtons",
  );
  const globalDiscountTypeBtns = globalDiscountTypeButtons
    ? globalDiscountTypeButtons.querySelectorAll(".discount-type-btn")
    : [];
  const applyGlobalDiscountBtn = document.getElementById(
    "applyGlobalDiscountBtn",
  );
  const clearGlobalDiscountBtn = document.getElementById(
    "clearGlobalDiscountBtn",
  );
  ...
  async function loadSystemSettings() {
    try {
      const response = await fetch(
        "https://nodetest-backend-jwqo.onrender.com/settings",
      );
      if (response.ok) {
  ...
  function calculateChange() {
    const totalDue =
      parseFloat(totalUSDDisplay.textContent.replace("$", "")) || 0;
    ...
    let displayChangeKHR = Math.abs(changeKHR).toLocaleString("en-US", {
  ...
  function updatePaymentBtnActiveState() {
    [cashBtn, cardBtn, deliveryBtn].forEach((btn) =>
  ...
  function generateMenuGrid(data) {
    ...
    data.forEach((item, index) => {
      ...
      const priceToDisplay = item.isPromo
        ? item.promoPrice
        : item.originalPrice;
  ...
  function calculateOrderItemPrice(item) {
    ...
    if (item.isPromo) {
      const menuItem = menuData.find(
        (m) => isSameItemId(m._id, item.id) || isSameItemId(m.id, item.id),
      );
    ...
    effectivePrice =
      effectivePrice - Math.min(customDiscountAmount, effectivePrice);
  ...
  function calculateTotals() {
    ...
    if (discountDisplay)
      discountDisplay.textContent = `-$${totalAllDiscount.toFixed(2)}`;
    ...
    if (totalUSDDisplay)
      totalUSDDisplay.textContent = `$${finalUSD.toFixed(2)}`;
    if (totalKHRDisplay)
      totalKHRDisplay.textContent = `${finalKHR.toLocaleString("en-US")}៛`;

    // 5. Update Cash Modal if Open
    if (
      typeof cashModal !== "undefined" &&
      cashModal &&
  ...
  function updateOrderDisplay() {
    ...
    for (const uniqueId in order) {
      ...
      if (!menuItemData) {
        console.warn(
          "Menu item data not found for order item:",
          item.id,
          "Available:",
          menuData.map((m) => m._id?.toString() || m.id),
        );
        continue;
      }

      const unitPrice = menuItemData.originalPrice;
      const finalUnitPrice = item.isPromo
        ? menuItemData.promoPrice || unitPrice
        : unitPrice;
      ...
      const imageUrl =
        menuItemData.image ||
        "https://via.placeholder.com/50x50/EBEBEB/9E9E9E?text=IMG";
  ...
  cashModal.addEventListener("click", (e) => {
    if (
      e.target === cashModal &&
      cashModal.dataset.closeOnBackdrop !== "false"
    ) {
      resetCashModal();
  ...
  if (saveDeliveryAppBtn) {
    saveDeliveryAppBtn.addEventListener("click", () => {
      const selectedBtn = Array.from(deliveryAppBtns).find((btn) =>
  ...
  if (applyGlobalDiscountBtn) {
    applyGlobalDiscountBtn.addEventListener("click", () => {
      ...
      const type =
        Array.from(globalDiscountTypeBtns).find((btn) =>
  ...
  function updateItemModalDisplay(item) {
    ...
    // Set image source
    const itemModalImg = document.querySelector(
      "#itemDetailModal .item-modal-img",
    );
    if (itemModalImg) {
  ...
  function handleMenuItemClick(e) {
    ...
    const itemData = menuData.find(
      (item) => isSameItemId(item._id, itemId) || isSameItemId(item.id, itemId),
    );

    if (!itemData) {
      console.log("Item data not found for ID:", itemId);
      console.log(
        "Available menu data IDs:",
        menuData.map((m) => ({
          id: m.id,
          _id: m._id,
          name: m.name,
  ...
  function handleItemDetailSave(split = false) {
    ...
    const newQuantity =
      parseInt(itemModalQuantity.textContent.replace("x", "")) || 1;
    ...
    const newSugarLevel =
      Array.from(sugarSelectionBtns).find((btn) =>
        btn.classList.contains("selected"),
  ...
  if (modalQtyDecreaseBtn) {
    modalQtyDecreaseBtn.addEventListener("click", () => {
      ...
      const currentQty =
        parseInt(itemModalQuantity.textContent.replace("x", "")) || 0;
  ...
  if (saveItemDetailsBtn) {
    saveItemDetailsBtn.addEventListener("click", () =>
      handleItemDetailSave(false),
    );
  }
  if (splitOrderItemBtn) {
    splitOrderItemBtn.addEventListener("click", () =>
      handleItemDetailSave(true),
    );
  }
  ...
  function filterMenu(category) {
    ...
    console.log(
      `✅ POS: Filter found ${filteredData.length} items for "${category}"`,
    );
  ...
  if (viewAllCategories) {
    viewAllCategories.addEventListener("click", (e) => {
      ...
      const isViewAllMode =
        menuCategoriesContainer.classList.contains("view-all-mode");
  ...
  function generateInvoiceList(invoiceData = invoices) {
    ...
    invoiceData.forEach((invoice) => {
      ...
      const paymentMethodDisplay =
        invoice.status === "pending" ? "payment-pending" : paymentMethodClass;
...
`;
  ...
  async function showInvoiceDetailModal(invoice) {
    ...
    try {
      ...
      const invoiceRate =
        ...
        (typeof KHR_RATE !== "undefined" ? KHR_RATE : 4000);
      ...
      const endTimestamp = invoice.lastModifiedAt
        ? new Date(invoice.lastModifiedAt)
        : new Date();
      const endTime = endTimestamp.toLocaleTimeString("en-US", {
        hour12: true,
        hour: "2-digit",
        minute: "2-digit",
      });
      const durationMin = Math.max(
        0,
        Math.floor((endTimestamp - dateObj) / 60000),
      );
      ...
      const invoiceId =
        invoice.invoiceId || invoice._id?.substring(0, 8) || "0000";
      const waitingNum = invoiceId.slice(-4);

      // FORMAT PAYMENT TEXT FOR DELIVERY APPS
      const paymentMethodText =
      ...
      // 7. Render 100% Original HTML Structure
    ...
    try {
      ...
      if (invoice) {
        ...
        if (invoice.status !== "pending") {
          alert(
            "Cannot edit this invoice. Only pending invoices can be edited.\n\nPaid invoices can only be modified by administrators.",
          );
          return;
        }
      ...
      showNotification(
        "error",
        "Permission Denied: Only pending invoices can be edited.",
      );
    }
  }
      ...
      try {
        ...
        const invoiceData = {
          ...
          items: Object.values(order).map((item) => {
            const menuItem = menuData.find(
              (m) =>
                isSameItemId(m._id, item.id) || isSameItemId(m.id, item.id),
            );
            return {
              id: item.id,
              name: item.name,
              ...
              price: item.isPromo
                ? menuItem.promoPrice || menuItem.originalPrice
          ...
          subtotal:
            parseFloat(subTotalDisplay.textContent.replace("$", "")) || 0,
        ...
        if (currentEditingInvoiceId) {
          ...
          newInvoice = await api.updateInvoice(
  ...
  if (document.getElementById("finalizeCashPayment")) {
    document
      .getElementById("finalizeCashPayment")
      .addEventListener("click", async () => {
        ...
        if (totalReceivedUSD < totalDue - 0.01) {
          ...
          showNotification(
            "error",
            "Insufficient payment! Please check amount.",
          );
          return;
        }
  ...
  function updatePOSBranding() {
    ...
    if (typeof systemSettings !== "undefined") {
      ...
      if (
        logoEl &&
        systemSettings.receiptLogo &&
        systemSettings.receiptLogo.trim() !== ""
      ) {
  ...
  async function loadCategories() {
    try {
      ...
      if (!response.ok) {
        console.error(
          `❌ POS: API error: ${response.status} ${response.statusText}`,
        );
        return [];
      }
  ...
  function refreshPOS() {
    ...
    if (menuCategoriesContainer)
      menuCategoriesContainer.innerHTML = "<div>Loading categories...</div>";
  ...
  async function forceRefreshCategories() {
    ...
    if (menuCategoriesContainer) {
      menuCategoriesContainer.innerHTML =
        '<div style="padding: 10px; text-align: center;">Refreshing categories...</div>';
    }
  ...
  async function showEndOfDayReport() {
    try {
      ...
      const todaysInvoices = invoices.filter((inv) => {
        ...
        const isMyInvoice =
          inv.createdBy === currentCashier || currentCashier === "admin";

        return isToday && isPaid && isMyInvoice;
      });

      // 5. Calculate Revenue (Including Delivery)
      const cashRevenue = todaysInvoices
        .filter((i) => (i.paymentMethod || "cash").toLowerCase() === "cash")
        .reduce((sum, i) => sum + (i.total || 0), 0);
      const cardRevenue = todaysInvoices
        .filter((i) => {
      ...
      const deliveryRevenue = todaysInvoices
        .filter((i) => {
      ...
      todaysInvoices.forEach((inv) => {
        if (inv.items) {
          inv.items.forEach((item) => {
            ...
            let unitPrice =
              priceMap[name] || item.originalPrice || item.price || 0;
      ...
      const itemRowsHtml = sortedItems
        .map(
          (item) => `
            <tr>
  ...
  function updateMobileCartCount() {
    const count = Object.values(order).reduce(
      ...
      if (response.ok) {
        if (window.showNotification) {
          window.showNotification(
            "success",
            `Shift report for ${currentCashier} sent to Telegram!`,
          );
        }
      }
    } catch (error) {
